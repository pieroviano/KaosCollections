...
class RsExample06
{
    ...
    static void Main()
    {
        var names1 = new[] { "Falco", "Nico", "David Bowie", "Tom Petty", "Joni Mitchell", "Warren Zevon" };
        var names2 = new[] { "Michelangelo", "Rembrandt", "Joni Mitchell", "David Bowie" };
        ...
        foreach (var name in musicians)
            Console.WriteLine ($"  {name}");

        var comp = RankedSet<string>.CreateSetComparer();

        var setOfSets = new HashSet<RankedSet<string>> (comp)
        {
            musicians,
            painters
        };
