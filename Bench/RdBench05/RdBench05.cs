...
class RdBench05
{
    ...
    static void Main()
    {
        ...
        for (var group = 1; group <= 10; ++group)
        {
            ...
            var rdMs = RunAddSequential (rd, group, scale);
            var sdMs = RunAddSequential (sd, group, scale);
            sumRd += rdMs; sumSd += sdMs;

            var rdSMs = RunSeek (rd, group * scale);
            var sdSMs = RunSeek (sd, group * scale);

            var rdEMs = RunKeyIterate (rd);
            var sdEMs = RunKeyIterate (sd);

            Console.WriteLine ($"{rdMs,6};{sdMs,6};{rdSMs,6};{sdSMs,6};{rdEMs,6};{sdEMs,6}");

            sumChange += sumRd / (float) sumSd;
        }
        var change = sumChange * 10 + 0.5F;
