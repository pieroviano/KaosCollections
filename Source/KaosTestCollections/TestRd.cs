...
public partial class TestRd : TestBtree, IClassFixture<BinaryFormatterEnableFixture>
{
    ...
    public void UnitRd_CtorSubclass()
    {
        ...
        var isRO = ((System.Collections.Generic.IDictionary<int, int>)sub).IsReadOnly;
        Assert.False(isRO);
    }
    ...
    public void CrashRd_Remove_ArgumentNull()
    {
        Assert.Throws<ArgumentNullException>(() =>
        {
            Setup();
            dary2.Add("delta", 4);
            var isRemoved = dary2.Remove(null);
        });
    }
    ...
    public void UnitRd_Remove()
    {
        ...
        foreach (var key in iVals1)
            dary1.Add(key, key + 1000);
        var c0 = dary1.Count;
        var isRemoved1 = dary1.Remove(iVals1[3]);
        var isRemoved2 = dary1.Remove(iVals1[3]);
    ...
    public void CrashRd_TryGetValue_ArgumentNull()
    {
        ...
        {
            Setup();
            dary2.TryGetValue(null, out var resultValue);
        });
    }
    ...
    public void UnitRd_TryGetValueOrUnfoundKeyInt()
    {
        ...
        for (var i = 2; i <= 50; i += 2)
            dary1.Add(i, i + 300);
        var result1 = dary1.TryGetValue(5, out var val1);
        var result2 = dary1.TryGetValue(18, out var val2);
        var result3 = dary1.TryGetValue(26, out var val3);
    ...
    public void UnitRd_TryGetValueForUnfoundKeyString()
    {
        ...
        for (var c = 'A'; c <= 'Z'; ++c)
            sd.Add(c.ToString(), c);
        var result1 = sd.TryGetValue("M", out var val1);
        var result2 = sd.TryGetValue("U", out var val2);
        var result3 = sd.TryGetValue("$", out var val3);
        Assert.Equal(val1, 'M');
        Assert.Equal(val2, 'U');
        Assert.True(result1);
        Assert.True(result2);
    ...
    public void CrashRd_oeCurrent_InvalidOperationA()
    {
        Assert.Throws<InvalidOperationException>(() =>
        {
            ...
            var zz = ((System.Collections.IEnumerator)kvEtor).Current;
        });
    }
    ...
    public void UnitRd_pcRemovePairNull()
    {
        ...
        var isOK = gpc.Remove(new KeyValuePair<int, string>(99, null));
        Assert.False(isOK);
    ...
    public void UnitRdx_ElementsBetweenPassedEnd()
    {
        ...
        var iterations = 0;
        var sumVals = 0;
        foreach (var e in rd.ElementsBetween(500, 1500))
        {
            ++iterations;
            sumVals += e.Value;
        }
    ...
    public void UnitRdx_IndexOfKey()
    {
        ...
        for (var ii = 0; ii < 500; ii += 2)
        {
            var ix = rd.IndexOfKey(ii);
            Assert.Equal(ii / 2, ix);
        }

        var iw = rd.IndexOfKey(-1);
        Assert.Equal(~0, iw);
        var iy = rd.IndexOfKey(500);
        Assert.Equal(~250, iy);
    }
    ...
    {
        Setup(4);
        var rem0 = dary1.RemoveAll(new[] { 2 });
        Assert.Equal(0, rem0);
        foreach (var ii in new[]
            ...
            dary1.Add(ii, -ii);
        var rem2 = dary1.RemoveAll(new[] { 2 });
        Assert.Equal(0, rem0);
        var rem3 = dary1.RemoveAll(new[] { 3, 4, 7, 7, 11 });
        Assert.Equal(2, rem3);
        Assert.True(System.Linq.Enumerable.SequenceEqual(new[] { 1, 5, 9 }, dary1.Keys));
        var rem4 = dary1.RemoveAll(dary1.Keys);
        Assert.Equal(3, rem4);
        Assert.Equal(0, dary1.Count);
    }
    ...
    public void CrashRdx_RemoveAtA_ArgumentOutOfRange()
    {
        Assert.Throws<ArgumentOutOfRangeException>(() =>
        {
            var rd = new RankedDictionary<int, int> { { 42, 24 } };
            rd.RemoveAt(-1);
        });
    }
    ...
    public void UnitRdx_TryGetGEGT()
    {
        ...
        var r0a = rd.TryGetGreaterThan("CC", out var p0a);
        Assert.False(r0a);
        Assert.Equal(default(string), p0a.Key);
        Assert.Equal(default(int? ), p0a.Value);
        var r0b = rd.TryGetGreaterThanOrEqual("DD", out var p0b);
        Assert.False(r0b);
        Assert.Equal(default(string), p0b.Key);
        Assert.Equal(default(int? ), p0b.Value);
        var r1 = rd.TryGetGreaterThan("BB", out var p1);
        Assert.True(r1);
        Assert.Equal("CC", p1.Key);
        Assert.Equal(2, p1.Value);
        var r2 = rd.TryGetGreaterThanOrEqual("BB", out var p2);
        Assert.True(r2);
        Assert.Equal("BB", p2.Key);
        Assert.Equal(1, p2.Value);
        var r3 = rd.TryGetGreaterThanOrEqual("AA", out var p3);
        Assert.True(r3);
    ...
    public void UnitRdx_TryGetLELT()
    {
        ...
        var r0a = rd.TryGetLessThan("BB", out var p0a);
        Assert.False(r0a);
        Assert.Equal(default(string), p0a.Key);
        Assert.Equal(default(int? ), p0a.Value);
        var r0b = rd.TryGetLessThanOrEqual("AA", out var p0b);
        Assert.False(r0b);
        Assert.Equal(default(string), p0b.Key);
        Assert.Equal(default(int? ), p0b.Value);
        var r1 = rd.TryGetLessThan("CC", out var p1);
        Assert.True(r1);
        Assert.Equal("BB", p1.Key);
        Assert.Equal(1, p1.Value);
        var r2 = rd.TryGetLessThanOrEqual("CC", out var p2);
        Assert.True(r2);
        Assert.Equal("CC", p2.Key);
        Assert.Equal(2, p2.Value);
        var r3 = rd.TryGetLessThanOrEqual("DD", out var p3);
        Assert.True(r3);
    ...
    public void UnitRdx_TryGetValueIndex()
    {
        ...
        for (var ii = 0; ii < 500; ii += 2)
        {
            var isOk = rd.TryGetValueAndIndex(ii, out var v1, out var i1);
            Assert.True(isOk);
            Assert.Equal(ii / 2, i1);
            Assert.Equal(ii + 1000, v1);
        }

        var isOkNot = rd.TryGetValueAndIndex(111, out var v2, out var i2);
    ...
    public void UnitRdx_RemoveWhereA()
    {
        ...
        for (var ix = 0; ix < 1000; ++ix)
            rd.Add(ix, ix + 1000);
        var c0 = rd.Count;
        var removed = rd.RemoveWhere(IsEven);
    ...
    public void UnitRdx_RemoveWhereB()
    {
        ...
        for (var ix = 0; ix < n; ++ix)
            rd.Add(ix, -ix);
        var removed = rd.RemoveWhere(IsAlways);
    ...
    public void UnitRdx_RemoveWhereElement()
    {
        ...
        for (var ix = 0; ix < 1000; ++ix)
            rd.Add(ix, -ix);
        var c0 = rd.Count;
        var removed = rd.RemoveWhereElement(IsPairEven);
        Assert.Equal(500, removed);
